...
## README.md + LICENSE + FONTS_LICENSE (bundled M PLUS 1p font), and
## notarize the darwin build. Asset naming follows the org Release
## Archive Standard (json-to-table-vX.Y.Z-<os>-<arch>.<ext>).
package: build-all
...
BREW_KIND := formula
BREW_DESC := Render JSON arrays as text, Markdown, or CSV tables
